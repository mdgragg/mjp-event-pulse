...
export async function getEventSpeakers(id) {
  const data = await fetchAPI(
    `
  ...
  return data.events[0].EventSpeakers;
}
export async function getBreakoutSessions(id) {
  const data = await fetchAPI(
    `  query Breakouts($id: ID!){
  events(where: { id: $id }) {
    BreakoutSessions {
      id
      Name
  ...
  return data.events[0].BreakoutSessions;
}
